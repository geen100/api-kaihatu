...
if page == 'users':
    ...
    with st.form(key='user'):
        ...
        submit_buttun = st.form_submit_button(label='リクエスト送信')
    ...
    st.title('APIテスト画面（自習室）')
    with st.form(key='room'):
        room_id: int =random.randint(0,10)
        room_name: str = st.text_input('自習室名',max_chars=12)
        ...
        submit_buttun = st.form_submit_button(label='送信')
